...
PROXMOX_PASSWORD = os.environ.get("PROXMOX_PASSWORD", "root")
...
def assign_pools(pve, created_cts):
    ...
    pool_assignments = {
        "production": POOL_VM_ASSIGNMENTS["production"]
        + [ct["vmid"] for ct in created_cts if ct["vmid"] <= 203],  # ty:ignore[unsupported-operator]
        "staging": POOL_VM_ASSIGNMENTS["staging"]
        + [ct["vmid"] for ct in created_cts if ct["vmid"] == 204],
        "development": POOL_VM_ASSIGNMENTS["development"]
...
def trigger_tasks(node):
    ...
    created_tasks = []
    for vmid in TASK_START_VMIDS:
        try:
            task = node.qemu(vmid).status.start.post()
